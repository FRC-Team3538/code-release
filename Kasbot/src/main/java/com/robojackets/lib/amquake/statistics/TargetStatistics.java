package com.robojackets.lib.amquake.statistics;
...
public class TargetStatistics extends Statistics<PhotonTrackedTarget> {
  ...
  public void update(PhotonTrackedTarget target, double timestampSeconds) {
    update(
        new Pose3d().plus(target.getBestCameraToTarget()),
  ...
  public Translation3d getTranslationAvg() {
    return poseStats.getTranslationAvg();
  }

  public Translation3d getTranslationStdDev() {
    return poseStats.getTranslationStdDev();
  }

  public Rotation3d getRotationAvg() {
    return poseStats.getRotationAvg();
  }

  public double[] getRotationStdDev() {
    return poseStats.getRotationStdDev();
  }

  public List<TargetCorner> getCornerAvgs() {
    return cornerStats.getCornerAvgs();
  }

  public List<TargetCorner> getCornerStdDevs() {
    return cornerStats.getCornerStdDevs();
  }

  public TargetCorner getTotalCornerStdDev() {
    return cornerStats.getTotalCornerStdDev();
  }

  public TargetCorner getTotalCornerSqError() {
    return cornerStats.getTotalCornerSqError();
  }
}
