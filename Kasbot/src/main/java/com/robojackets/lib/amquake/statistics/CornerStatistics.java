...
public class CornerStatistics extends Statistics<List<TargetCorner>> {
  ...
  public void setBufferSeconds(double bufferSeconds) {
    ...
    if (cornerBuffers != null)
      for (var buffer : cornerBuffers) buffer.setBufferSeconds(bufferSeconds);
  }
  ...
  public void update(List<TargetCorner> corners, double timestamp) {
    ...
    for (int i = 0; i < corners.size(); i++) {
      if (cornerBuffers.size() <= i)
        cornerBuffers.add(new TimedBuffer<TargetCorner>(bufferSeconds));
  ...
  private void calculate() {
    ...
    for (var buffer : cornerBuffers) {
      ...
      var stdDev =
          new TargetCorner(
              Math.sqrt(sqErr.x / corners.length), Math.sqrt(sqErr.y / corners.length));
      cornerAvgs.add(avg);
  ...
  public static List<TargetCorner> allTargetCorners(List<PhotonTrackedTarget> targets) {
    var corners = new ArrayList<TargetCorner>();
    targets.stream()
        .sorted((a, b) -> a.getFiducialId() - b.getFiducialId())
        .forEach(t -> corners.addAll(t.getDetectedCorners()));
    return corners;
  }
}
