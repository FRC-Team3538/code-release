...
public class Telemetry {
  ...
  public void telemeterize(SwerveDriveState state) {
    ...
    fieldPub.set(
        new double[] {pose.getX(), pose.getY(), pose.getRotation().getZ() * 180 / Math.PI});

    /* Telemeterize the robot's general speeds */
    speed.set(
        Math.sqrt(
            state.speeds.vxMetersPerSecond * state.speeds.vxMetersPerSecond
                + state.speeds.vyMetersPerSecond * state.speeds.vyMetersPerSecond));
    velocityX.set(state.speeds.vxMetersPerSecond);
    velocityY.set(state.speeds.vyMetersPerSecond);
    velocityW.set(state.speeds.omegaRadiansPerSecond);
