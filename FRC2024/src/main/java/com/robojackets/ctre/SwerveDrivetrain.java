...
public class SwerveDrivetrain {
  ...
  protected Rotation3d m_orientation = new Rotation3d();
  ...
  public static class SwerveDriveState {
    ...
    public Pose3d Pose;
    /** The current local-frame velocity of the robot */
  ...
  public class OdometryThread {
    ...
    public OdometryThread() {
      ...
      m_allSignals = new BaseStatusSignal[(ModuleCount * 4) + 11];
      ...
      m_allSignals[m_allSignals.length - 11] = m_pigeon2.getYaw();
      m_allSignals[m_allSignals.length - 10] = m_pigeon2.getAngularVelocityXDevice();
      m_allSignals[m_allSignals.length - 9] = m_pigeon2.getAngularVelocityYDevice();
      m_allSignals[m_allSignals.length - 8] = m_pigeon2.getAngularVelocityZDevice();
      m_allSignals[m_allSignals.length - 7] = m_pigeon2.getAngularVelocityXWorld();
      m_allSignals[m_allSignals.length - 6] = m_pigeon2.getAngularVelocityYWorld();
      m_allSignals[m_allSignals.length - 5] = m_pigeon2.getAngularVelocityZWorld();
      m_allSignals[m_allSignals.length - 4] = m_pigeon2.getQuatW();
      m_allSignals[m_allSignals.length - 3] = m_pigeon2.getQuatX();
      m_allSignals[m_allSignals.length - 2] = m_pigeon2.getQuatY();
      m_allSignals[m_allSignals.length - 1] = m_pigeon2.getQuatZ();
    }
      ...
      while (m_running) {
        ...
        try {
          ...
          if (Utils.isSimulation()) {
            ...
            m_orientation = new Rotation3d(0, 0, yawDegrees * Math.PI / 180);
          } else {
            var orientation =
                new Quaternion(
            ...
            //     new Quaternion(
            //         0,
            //         m_pigeon2.getAngularVelocityXDevice().getValueAsDouble(),
            //         m_pigeon2.getAngularVelocityYDevice().getValueAsDouble(),
            //         m_pigeon2.getAngularVelocityZDevice().getValueAsDouble());

            m_orientation = new Rotation3d(orientation);
            // new Rotation3d(
            //     angularVelocity
            //         .times(m_pigeon2.getQuatW().getTimestamp().getLatency() / 2)
            //         .exp()
            //         .times(orientation));
          }

          /* Keep track of previous and current pose to account for the carpet vector */
          m_odometry.update(m_orientation, m_modulePositions);
          ...
          m_requestParameters.currentPose = m_odometry.getEstimatedPosition3d().toPose2d();
          ...
          try {
            Logger.recordOutput("Swerve/Request", m_requestToApply.getClass().getSimpleName());
          } catch (Exception e) {
          }
          ...
          m_cachedState.Pose = m_odometry.getEstimatedPosition3d();
  ...
  public SwerveDrivetrain(
      SwerveDrivetrainConstants driveTrainConstants,
      double OdometryUpdateFrequency,
      Matrix<N6, N1> odometryStandardDeviation,
      Matrix<N6, N1> visionStandardDeviation,
      SwerveModuleConstants... modules) {
    ...
    m_yawGetter = m_pigeon2.getYaw();
    m_angularVelocity = m_pigeon2.getAngularVelocityZWorld();
    ...
    m_cachedState.ModuleStates = new SwerveModuleState[ModuleCount];
    m_cachedState.ModuleTargets = new SwerveModuleState[ModuleCount];
    m_cachedState.Pose = m_odometry.getEstimatedPosition3d();
    m_cachedState.speeds = new ChassisSpeeds();

    m_requestParameters.currentChassisSpeed = new ChassisSpeeds();
    m_requestParameters.currentPose = new Pose2d();
    m_requestParameters.operatorForwardDirection = m_operatorForwardDirection;
  ...
  public void tareEverything() {
    try {
      ...
      m_odometry.resetPosition(
          m_orientation,
          m_modulePositions,
          new Pose3d(new Translation3d(), AllianceFlipUtil.apply(new Rotation3d())));
    } finally {
      m_stateLock.writeLock().unlock();
    }
  }
  ...
  public void seedFieldRelative() {
    try {
      ...
      var pose = m_odometry.getEstimatedPosition3d();

      m_odometry.resetPosition(
          m_orientation,
          m_modulePositions,
          new Pose3d(pose.getTranslation(), AllianceFlipUtil.apply(new Rotation3d())));
  ...
  public void setOperatorForwardToCurrentDirection() {
    try {
      m_stateLock.writeLock().lock();

      m_operatorForwardDirection = m_cachedState.Pose.toPose2d().getRotation();
    } finally {
      m_stateLock.writeLock().unlock();
    }
  }
   ...
   */
  public void seedFieldRelative(Pose3d location) {
    try {
      m_stateLock.writeLock().lock();

      m_odometry.resetPosition(m_orientation, m_modulePositions, location);
    } finally {
      m_stateLock.writeLock().unlock();
    }
  }

  public Pose3d samplePoseAt(double timestamp) {
    try {
      m_stateLock.writeLock().lock();

      return m_odometry.sampleAt(timestamp);
    } finally {
      m_stateLock.writeLock().unlock();
    }
  }
  ...
  public boolean isWithinBuffer(double timestamp) {
    try {
      ...
      return m_odometry.isWithinBuffer(timestamp);
    } finally {
      m_stateLock.writeLock().unlock();
    }
  }
  ...
  /**
   * Gets the current control request parameters of the swerve drivetrain.
   *
   * @return Current control request of the drivetrain
   */
  public SwerveControlRequestParameters getControlParameters() {
    try {
      m_stateLock.readLock().lock();
  ...
  public void setVisionMeasurementStdDevs(Matrix<N6, N1> visionMeasurementStdDevs) {
    try {
      m_stateLock.writeLock().lock();
      m_odometry.setVisionMeasurementStdDevs3d(visionMeasurementStdDevs);
    } finally {
      m_stateLock.writeLock().unlock();
    }
  }
