...
public class LimelightModule extends VisionModule {
  ...
  public Optional<VisionEstimate> getNewPoseEstimate() {
    ...
    Logger.recordOutput(String.format("vision/%s/latency", limelight), total_latency);
    Logger.recordOutput(
    ...
    Logger.recordOutput(String.format("vision/%s/timestamp", limelight), estimate_timestamp);
    Logger.recordOutput(
