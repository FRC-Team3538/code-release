...
public class AllianceFlipUtil {
  ...
  /** Flips chassis speeds to the correct side of the field based on the current alliance color. */
  public static ChassisSpeeds apply(ChassisSpeeds speeds) {
    if (shouldFlip()) {
      return ChassisSpeeds.fromFieldRelativeSpeeds(speeds, Rotation2d.fromRotations(0.5));
    } else {
      return speeds;
    }
  }
   ...
   * Flips a trajectory state to the correct side of the field based on the current alliance color.
   */
  public static TrajectoryState apply(TrajectoryState state) {
    if (shouldFlip()) {
      var pose = apply(state.getPose());
  ...
  private static boolean shouldFlip() {
    return DriverStation.getAlliance().filter(Alliance.Red::equals).isPresent();
  }
}
