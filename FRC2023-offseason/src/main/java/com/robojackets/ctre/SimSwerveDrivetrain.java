...
public class SimSwerveDrivetrain {
  ...
  protected final SwerveDriveWheelPositions m_lastPositions;
  ...
  public SimSwerveDrivetrain(
    ...
    m_lastPositions = new SwerveDriveWheelPositions(new SwerveModulePosition[ModuleCount]);
    for (int i = 0; i < ModuleCount; ++i) {
      ...
      m_lastPositions.positions[i] = new SwerveModulePosition(0, new Rotation2d());
    }
  ...
  public void update(double dtSeconds, double supplyVoltage, SwerveModule... modulesToApply) {
    ...
    SwerveDriveWheelPositions positions =
        new SwerveDriveWheelPositions(new SwerveModulePosition[ModuleCount]);
    ...
    for (int i = 0; i < ModuleCount; ++i) {
      ...
      var pos = modulesToApply[i].getPosition(true);
      positions.positions[i] = new SwerveModulePosition(pos.distanceMeters, pos.angle);
    }

    double angleChange = Kinem.toTwist2d(m_lastPositions, positions).dtheta;
    LastAngle = LastAngle.plus(Rotation2d.fromRadians(angleChange));
    PigeonSim.setRawYaw(LastAngle.getDegrees());

    m_lastPositions.positions = positions.positions;
  }
